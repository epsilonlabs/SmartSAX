...
public class EffectiveMetamodelReconciler {
				...
				if (eClassifier instanceof EClass) {
					...
					if (actualObjectToLoad(ePackage, (EClass) eClassifier)) {
						...
						for (EReference eReference : leClass.getEAllReferences()) {
							if (actualObjectsAndFeaturesToLoad.get(ePackage.getName()).get(eClassifier.getName())
									.contains(eReference.getName())) {
					...
					if (typesToLoad(ePackage, leClass)) {
						for (EReference eReference : leClass.getEAllReferences()) {
							if (typesToLoad.get(ePackage.getName()).get(eClassifier.getName())
									.contains(eReference.getName())) {
								EClass eType = (EClass) eReference.getEType();
								addTypesToLoad(eType);
							}
		...
		for (EffectiveMetamodel em : effectiveMetamodels) {
			...
			if (em.getName().equalsIgnoreCase(ePackage.getName())) {
				...
				for (EffectiveType et : em.getAllOfKind()) {
					...
					// if the element's name is equal to the eclass's name,
					// return true
					...
					// if the eclass's super types contains the type also return
					// true
	...
	public void addActualObjectToLoad(EClass eClass) {
		...
		if (subMap == null) {
			...
			// if refs is null, create new refs and add the ref and then add to
			// sub map
			if (refs == null) {
		...
		for (EffectiveMetamodel em : effectiveMetamodels) {
			...
			if (em.getName().equals(ePackage.getName())) {
				for (EffectiveType et : em.getTypes()) {
					...
					// if eclass is a sub class of the kind, add all attributes
					// and references
	...
	// determines if a class is live class, this is used to generate the
	// traversal path
		...
		for (EffectiveMetamodel em : effectiveMetamodels) {
			...
			if (em.getName().equalsIgnoreCase(ePackage.getName())) {
				...
				for (EffectiveType et : em.getAllOfKind()) {
					...
					// if the current class under question is a sub class of the
					// mec, should return true
					if (actual.getEAllSuperTypes().contains(kind)) {
						return true;
					}
					// if the current class under question is a super class of
					// the mec, should also return true
				...
				for (EffectiveType et : em.getAllOfType()) {
					...
					EClass actual = (EClass) ePackage.getEClassifier(className);
					// if the class under question is a super class of the mec,
					// should return true
				...
				if (placeHolders != null) {
					for (String t : placeHolderObjects.get(em.getName())) {
						...
						// get the eclass for the class under question
						EClass actual = (EClass) ePackage.getEClassifier(className);
						// if the class under question is a super class of the
						// mec, should return true
	...
	// add classes and references to visit and fill up the
	// objectsAndRefNamesToVisit map
	...
	public boolean liveReference(EReference eReference) {
		// if reference is containment, we are not looking into non-containment
		// references
