...
public class SmartSAXXMIHandler extends SAXXMIHandler {
	...
	public void setTypesToLoad(HashMap<String, HashMap<String, ArrayList<String>>> typesToLoad) {
	...
	public void setActualObjectsToLoad(HashMap<String, HashMap<String, ArrayList<String>>> actualObjectsToLoad) {
		this.actualObjectsAndFeaturesToLoad = actualObjectsToLoad;
	}

	public SmartSAXXMIHandler(XMLResource xmiResource, XMLHelper helper, Map<?, ?> options) {
	...
	public boolean shouldProceed(EClass eClass, String name) {
		// if current class is not null and current class is equal to the class
		// in question
	...
	protected EObject createDocumentRoot(String prefix, String uri, String name, EFactory eFactory, boolean top) {
		...
		if (extendedMetaData != null && eFactory != null) {
			...
			if (eClass != null) {
				...
				EObject newObject = useNewMethods ? createObject(eFactory, eClass, true)
						: helper.createObject(eFactory, typeName);
				validateCreateObjectFromFactory(eFactory, typeName, newObject);
				if (top) {
					if (suppressDocumentRoot) {
						// Set up a deferred extent so the document root we
						// create definitely will not be added to the resource.
						//
						...
						// Remove the document root's information from the top
						// of the stacks.
						//
						objects.remove(0);
						...
						if (peekObject == null) {
							...
							if (objects.size() > 1) {
								...
								// If there is no root object, we're dealing
								// with an EAttribute feature instead of an
								// EReference feature.
								// So create an instance of simple any type and
								// prepare it to handle the text content.
								//
						...
						// Do the extent processing that should have been done
						// for the root but was actualljy done for the document
						// root.
						//
						...
						// The new root object is the actual new object since
						// all sign of the document root will now have
						// disappeared.
						//
	...
	protected void setFeatureValue(EObject object, EStructuralFeature feature, Object value) {
		if (shouldHandleFeature(object, feature.getName())) {
			...
			// if (handlingFeature && shouldHandleFeatureForType(object,
			// feature.getName())) {
			// setFeatureValue(object, feature, value, -1);
			// }
